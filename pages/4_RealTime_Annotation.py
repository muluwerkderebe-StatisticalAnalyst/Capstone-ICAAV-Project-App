...
# Keep the complete functional workflow while presenting each stage in the
# compact tab structure used by the preferred Page 4 design.
tabs = st.tabs(
    [
        "1. Data Import",
        "2. Model Import",
...
if save_btn:
    ...
    if existing:
        updated_annotations = list(st.session_state.annotation_data)
        for a in updated_annotations:
            if a["index"] == current_i:
                a["label"] = selected_label
        st.session_state.annotation_data = updated_annotations
        save_status.success(f"Updated: Row {current_i} to {selected_label}")
    else:
        st.session_state.annotation_data = st.session_state.annotation_data + [{
            "index": current_i,
...
if not annotations:
    st.info("Save at least one annotation in Tab 5 to enable the final downloads.")
else:
    ann_df = (
    ...
    # Merge the latest manual label back onto the original recorded dataset.
    # Annotation-only export also remains available for live UDP row indices.
    export_df = df.copy().reset_index(drop=True)
    annotation_lookup = ann_df.set_index("index")["label"].to_dict()
    export_df["corrected_label"] = [
        annotation_lookup.get(row_index, "")
        for row_index in range(len(export_df))
    ]

    st.metric("Saved Annotations", f"{len(ann_df):,}")
    st.subheader("Final Annotated Dataset Preview")
    st.dataframe(export_df.head(25), use_container_width=True)

    annotations_csv = ann_df.to_csv(index=False).encode("utf-8-sig")
    annotated_dataset_csv = export_df.to_csv(index=False).encode("utf-8-sig")
    ...
    with exp_col1:
        st.download_button(
            ...
            mime="text/csv",
            key="download_annotation_log",
            use_container_width=True,
        )
    with exp_col2:
        st.download_button(
            "Download Final Annotated Dataset",
            data=annotated_dataset_csv,
            file_name="dataset_with_corrected_labels.csv",
            mime="text/csv",
            key="download_final_annotated_dataset",
            use_container_width=True,
        )
