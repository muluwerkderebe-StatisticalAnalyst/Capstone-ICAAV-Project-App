...
with col2:
    st.markdown("""
        <h2 style='text-align: center; color: #B31B1B;'>
            Tab 4 — Real-Time Testing, Visualization, and Annotation
        </h2>
        <p style='text-align: center; color: gray;'>
            Offline Playback · Live Classification · Manual Label Correction
...
# -----------------------------
st.header("4. Live Visualization")
...
def render_monitor():
    ...
    if len(buf_df) > 0 and len(plot_cols) > 0:
        ...
        if time_col != "Row Index" and time_col in df.columns:
        ...
        class_colors = None
        if color_by != "None" and color_by in df.columns and len(buf_df) > 0:
        ...
        for idx, col_name in enumerate(plot_cols):
            ...
            if col_chart_type == "Line":
                ...
                ax.hist(y_vals, bins=20, color="steelblue", edgecolor="white", alpha=0.85)
...
def live_monitor():
    ...
    if idx >= len(df) - 1:
        ...
        if st.session_state.model is not None and len(model_feature_cols) > 0:
            try:
                feats = [float(r[c]) if c in numeric_df.columns else 0.0 for c in model_feature_cols]
...
st.caption("Pause playback to annotate the current row, then resume.")
...
with exp_col2:
    if len(st.session_state.annotation_data) > 0:
        export_df = df.copy()
        export_df["corrected_label"] = np.nan
